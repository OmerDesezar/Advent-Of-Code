...
const day3Part1 = (input: string[]) => {
	let sum = 0;
	const matrix = input.map((str) => str.split(""));
...
const recSearchPart1 = (matrix, i, j, valid = false, sum = 0) => {
	...
	if (matrix[i][j - 1] && matrix[i][j - 1] !== "." && Number.isNaN(Number(matrix[i][j - 1]))) {
		valid = true;
	}
	if (
		matrix[i + 1]?.[j - 1] &&
	...
	if (matrix[i - 1]?.[j] && matrix[i - 1][j] !== "." && Number.isNaN(Number(matrix[i - 1][j]))) {
		valid = true;
	}
	if (matrix[i + 1]?.[j] && matrix[i + 1][j] !== "." && Number.isNaN(Number(matrix[i + 1][j]))) {
		valid = true;
...
const searchAround = (matrix, i, j) => {
	...
	const set = new Set(arr.map((val) => JSON.stringify(val).replace("[", "").replace("]", "")));
